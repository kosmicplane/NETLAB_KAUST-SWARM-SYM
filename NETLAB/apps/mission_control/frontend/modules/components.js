...
export function barChart(entries, { width = 680, height = 230, label = '', unit = '', limit = 12 } = {}) {
  const data = (Array.isArray(entries) ? entries : Object.entries(entries || {}))
    ...
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit);
  if (!data.length) return '<div class="empty-state"><div><strong>No measured distribution</strong><div class="small-text">Bars appear after runtime samples exist.</div></div></div>';
...
export function empiricalCdf(values, { width = 680, height = 230, label = '', unit = '' } = {}) {
  ...
  if (!data.length) return '<div class="empty-state"><div><strong>No samples for empirical CDF</strong><div class="small-text">A distribution is shown only from measured samples.</div></div></div>';
  const pad = 30;
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(label)} empirical CDF"><line class="chart-grid" x1="${pad}" y1="${pad}" x2="${pad}" y2="${height-pad}"></line><line class="chart-grid" x1="${pad}" y1="${height-pad}" x2="${width-pad}" y2="${height-pad}"></line><text class="chart-label" x="${pad}" y="${height-8}">${escapeHtml(formatNumber(min,2))} ${escapeHtml(unit)}</text><text class="chart-label" x="${width-pad}" y="${height-8}" text-anchor="end">${escapeHtml(formatNumber(max,2))} ${escapeHtml(unit)}</text><polyline class="chart-line" points="${points}"></polyline></svg>`;
}

export function histogram(values, { bins = 12, width = 680, height = 230, label = '', unit = '' } = {}) {
  const data = values.map(Number).filter(Number.isFinite);
  if (!data.length) return '<div class="empty-state"><div><strong>No samples for histogram</strong><div class="small-text">Measured samples will populate this distribution.</div></div></div>';
  const min = Math.min(...data), max = Math.max(...data), range = max - min || 1;
  const counts = Array.from({ length: bins }, () => 0);
  for (const value of data) counts[Math.min(bins - 1, Math.floor(((value - min) / range) * bins))] += 1;
  return barChart(counts.map((count, index) => [`${formatNumber(min + index * range / bins, 1)}`, count]), { width, height, label, unit: 'samples', limit: bins });
}

export function linkBudgetWaterfall(sample = {}, { width = 680, height = 250 } = {}) {
  const stages = [
    ['TX power', Number(sample.tx_power_dbm)],
    ['TX gain', Number(sample.tx_gain_dbi)],
    ['Path loss', Number.isFinite(Number(sample.path_loss_db)) ? -Math.abs(Number(sample.path_loss_db)) : NaN],
    ['Environment', Number.isFinite(Number(sample.environmental_loss_db)) ? -Math.abs(Number(sample.environmental_loss_db)) : NaN],
    ['RX gain', Number(sample.rx_gain_dbi)],
    ['RX power', Number(sample.rx_power_dbm)],
  ].filter(([, value]) => Number.isFinite(value));
  if (!stages.length) return '<div class="empty-state"><div><strong>No link-budget components</strong><div class="small-text">The active link model has not emitted budget components.</div></div></div>';
...
export function eventTimeline(events = [], { width = 1000, height = 190 } = {}) {
  const normalized = events.map((event, index) => ({ ...event, _index: index, _time: Number(event.timestamp_wall ?? event.timestamp) })).filter(event => Number.isFinite(event._time));
  if (!normalized.length) return '<div class="empty-state"><div><strong>No runtime events</strong><div class="small-text">Outages, recovery, packet, and revision events will appear here.</div></div></div>';
  const min=Math.min(...normalized.map(e=>e._time)), max=Math.max(...normalized.map(e=>e._time)), range=max-min||1, pad=34;
  const markers=normalized.slice(-80).map((event,index)=>{const x=pad+((event._time-min)/range)*(width-2*pad);const lane=index%3;const y=55+lane*38;const type=String(event.event_type||'EVENT');const cls=/FAIL|OUTAGE|BLOCK/i.test(type)?'error':/RECOVER|PROMOT|DELIVER/i.test(type)?'success':/REVISION|SYNC/i.test(type)?'warning':'info';return `<g class="timeline-marker ${cls}"><line x1="${x.toFixed(1)}" y1="34" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}"></line><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="5"><title>${escapeHtml(type)}</title></circle><text class="chart-label" x="${x.toFixed(1)}" y="${y+15}" text-anchor="middle">${escapeHtml(type.slice(0,14))}</text></g>`}).join('');
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Packet outage and recovery event timeline"><line class="chart-grid" x1="${pad}" y1="34" x2="${width-pad}" y2="34"></line>${markers}</svg>`;
}
